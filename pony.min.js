;(function(context,undefined){var
    Pony=function(settings){var
        ...
        return this;},messageQueueObjectId=0,subscriptionToken=0,publish=function(kwargs){var
    messageQueue,instance=kwargs.instance,message=kwargs.message,data=kwargs.data,synchronousPublish=kwargs.synchronousPublish,subscribers=kwargs.subscribers,publicationObject,returnValues=[],subscribersLen,callbackQueue=[],len,i,throwException=function(exception){throw exception;},deliveredMessageCount=0,deliverMessage=function(subscriber,data){var
    returnValue;try{returnValue=subscriber.apply(context,data);}catch(e){setTimeout(throwException,0,e);return;}finally{deliveredMessageCount+=1;}
    if(synchronousPublish){returnValues.push(returnValue);}else if(instance.settings.queueMessages===true&&publicationObject){publicationObject.returnValues.push(returnValue);if(deliveredMessageCount==subscribers.length&&callbackQueue.length!==0){for(i=0,len=callbackQueue.length;i<len;i++){callbackQueue[i](publicationObject.returnValues);}}}};subscribers=subscribers||instance.subscriptionList[message];if(!subscribers||!subscribers.length){return false;}
    subscribersLen=subscribers.length;if(instance.settings.queueMessages===true){if(!instance.messageQueue.hasOwnProperty(message)){instance.messageQueue[message]=messageQueue=[];}else{messageQueue=instance.messageQueue[message];}
    ...
    if(synchronousPublish){while(subscribersLen--){deliverMessage(subscribers[subscribersLen],data);}}else{while(subscribersLen--){setTimeout(deliverMessage,0,subscribers[subscribersLen],data)}}
    ...
    if(this.settings.queueMessages===true&&this.messageQueue[message]&&this.messageQueue[message].length){messageQueueLen=this.messageQueue[message].length;while(messageQueueLen--){publish({instance:this,message:message,subscribers:this.messageQueue[message][messageQueueLen].data,synchronousPublish:false,subscribers:subscribers},null);}}
    ...
    data=Array.prototype.slice.call(arguments).slice(1);return publish({instance:this,message:message,data:data,synchronousPublish:false},null);},publishSync:function(message){var
    data=Array.prototype.slice.call(arguments).slice(1);return publish({instance:this,message:message,data:data,synchronousPublish:true},null);},startClearingMessageQueue:function(){var
